---
title: <center>Taller 7 AED</center>
...
# 1.
Considere los siguientes dos data sets:

$$\boldsymbol{X_1}=\left[\begin{array}{ll}
3\  \  7\\
2\  \  4\\
4\  \  7\\
\end{array}\right]$$
...
$$\boldsymbol{X_2}=\left[\begin{array}{ll}
6\  \  9\\
5\  \  7\\
4\  \  8\\
\end{array}\right]$$
...
$$\boldsymbol{\bar{X_1}}=\left[\begin{array}{ll}
3\\
6\\
\end{array}\right]$$

$$\boldsymbol{\bar{X_2}}=\left[\begin{array}{ll}
5\\
8\\
\end{array}\right]$$
...
$$\boldsymbol{S_{pooled}}=\left[\begin{array}{ll}
1 & 1\\
1 & 2\\
\end{array}\right]$$

$(a)$ Calcule la función discriminante de lineal $\hat{y}=(\bar{x_1}-\bar{x_2})'S_{pooled}^{-1}x=\hat{a}'x$
...
$(b)$ Clasifique la observación $x_0' = [2,7]$ como $\pi_1$ y $\pi_2$, usando ECM con iguales costos y probabilidades de principio.
...
cat("\nComo 4 es mayor a cero, a X0 la clasificamos en la población 1 \n")
```

$(c)$ Desarrolle una función de clasificación lineal usando $\hat{y}$ de $(a)$ 
...
$(d)$ Usando la función de $(c)$ y $\hat{m} =\frac{1}{2}(\bar{x_1}-\bar{x_2})'S_{pooled}^{-1}(\bar{x_1}+\bar{x_2})$, construya la matriz de confunsión.
...
$(e)$ Dados los resultados (d), calcule el APER.
```{r}
APER = (1+0)/6;
APER;
...
Suponga que $n_1 = 11$ y $n_2= 12$ son observaciones hechas de dos variables aleatorias $X_1$ y $X_2$, donde $X_1$ y $X_2$ se asumen de tener distribución normal bivariada, con una matriz de covarianza $\Sigma$, pero con posibles vectores de medias diferentes, $\mu_1$ y $\mu_2$ para dos poblaciones. Los vectores de medias y la covarianza es:
...
$(a)$ Construya la función discriminante de Fisher's: $\hat{y}=(\bar{x_1}-\bar{x_2})'S_{pooled}^{-1}x=\hat{a}'x$
...
$(b)$ Clasifique la observación $x_0' = [0,1]$ como $\pi_1$ y $\pi_2$, usando ECM con iguales costos y probabilidades de principio.
...
sprintf("Como %f es menor a cero, a X0 la clasificamos en la población 2", lado_izq_2)
```
